...
export default function AchievementToast({
  ...
  useEffect(() => {
    ...
    const countdown = animate(remainingTime, 0, {
      duration: remainingTime.get() * ACHIEVEMENT_NOTIFICATION_DURATION_MILLISECONDS / 1000,
      ease: "linear",
  ...
  return (
    <motion.aside
      ...
      animate={{ opacity: 1 }}
      className="pointer-events-auto bg-mapache-vivid-white text-mapache-vivid-black relative min-w-0 border-2 border-black shadow-[4px_4px_0px_0px_#000000]"
      onMouseEnter={() => onInteraction("hover", true)}
      onMouseLeave={() => onInteraction("hover", false)}
      onFocusCapture={() => onInteraction("focus", true)}
      onBlurCapture={event => {
        if (!event.currentTarget.contains(event.relatedTarget)) onInteraction("focus", false)
      }}
      onPointerDown={event => { if (event.pointerType === "touch") onInteraction("touch", true) }}
    ...
    >
      ...
      <div className="flow-root px-[8px] py-[4px]">
        ...
        >
          <svg aria-hidden="true" focusable="false" viewBox="0 0 20 20" className="size-[1em]" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="m6 6 8 8m0-8-8 8" />
          </svg>
        </button>
        <h2 className="text-sm leading-tight font-black [overflow-wrap:anywhere] uppercase">{achievement.title}</h2>
        <p className="text-sm leading-tight font-semibold [overflow-wrap:anywhere]">{achievement.unlockReason}</p>
      </div>
      <motion.div
        aria-hidden="true"
