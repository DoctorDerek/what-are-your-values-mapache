...
const preparationMachine = createSeethingSwarmAssetPreparationMachine<StaticImageData>()
const PreparationContext = createContext<ActorRefFrom<typeof preparationMachine> | undefined>(undefined)

export default function SeethingSwarmAssetPreparation({children}:{children:ReactNode}) {
  ...
  return <PreparationContext value={actor}>
    ...
    <div hidden aria-hidden="true">
      {[...assets.values()].map(({clip,generation})=><Image unoptimized key={generation} src={clip.asset} alt="" width={clip.frameWidth * clip.frameCount} height={clip.frameHeight} decoding="async" loading="eager" fetchPriority="low" onLoad={async event=>{
        ...
        try {
          await image.decode()
          actor.send({type:"ASSET.SETTLED",path:clip.relativePath,generation,status:"ready"})
        } catch {
          actor.send({type:"ASSET.SETTLED",path:clip.relativePath,generation,status:"failed"})
        }
      }} onError={()=>actor.send({type:"ASSET.SETTLED",path:clip.relativePath,generation,status:"failed"})} />)}
    </div>
  </PreparationContext>
}

export function usePreparedSeethingSwarmClips(clips:readonly SeethingSwarmRuntimeCharacterClip<StaticImageData>[]) {
...
export function usePreparedSeethingSwarmBattle(battle:PresentedBattle|null,catalog:SeethingSwarmRuntimeClipCatalog<StaticImageData>) {
  const actor = useContext(PreparationContext)
