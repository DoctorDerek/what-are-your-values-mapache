...
export default function SeethingSwarmBattleVariation({ children }: { children: ReactNode }) {
  const actor = useActorRef(seethingSwarmBattleVariationMachine)
  return <VariationContext value={actor}>{children}</VariationContext>
}

export function useSeethingSwarmProjectedBattle(battle: PresentedBattle | null, catalog: SeethingSwarmRuntimeClipCatalog<StaticImageData>) {
...
export function useSeethingSwarmActiveBattle(battle: PresentedBattle, catalog: SeethingSwarmRuntimeClipCatalog<StaticImageData>) {
  const actor = useContext(VariationContext)
  const scope = useId()
