...
export default function NativeAchievementBanner({
  ...
  useEffect(() => {
    const updateActivity = () => send({ type: "NOTIFICATION.APP_ACTIVITY", isActive: AppState.currentState === "active" })
    updateActivity()
    const activity = AppState.addEventListener("change", state => send({ type: "NOTIFICATION.APP_ACTIVITY", isActive: state === "active" }))
    const focus = AppState.addEventListener("focus", updateActivity)
    const blur = AppState.addEventListener("blur", () => send({ type: "NOTIFICATION.APP_ACTIVITY", isActive: false }))
    return () => { activity.remove(); focus.remove(); blur.remove() }
  }, [send])
  ...
  return (
    <View
      ...
      className={placement === "battle" ? "absolute top-0 right-3 left-3 z-50 items-center gap-2" : "absolute right-3 left-3 z-50 items-center gap-2"}
      style={placement === "screen" ? { bottom: safeAreaInsets.bottom + 12 } : undefined}
    >
