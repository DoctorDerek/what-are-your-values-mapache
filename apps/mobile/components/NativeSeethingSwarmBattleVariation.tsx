...
import {
  ...
  createSeethingSwarmChoreographyIdentity,
...
const VariationContext = createContext<
  ActorRefFrom<typeof seethingSwarmBattleVariationMachine> | undefined
>(undefined)

export default function NativeSeethingSwarmBattleVariation({
  children,
}: {
  children: ReactNode
}) {
...
export function useNativeSeethingSwarmProjectedBattle(
  ...
  const ordinals = useSelector(actor, (snapshot) =>
    projectSeethingSwarmBattleOrdinals(snapshot?.context, identity),
  )
  return useMemo(
    () =>
      battle
        ? createSeethingSwarmBattleChoreography({ battle, catalog, ordinals })
        : null,
    [battle, catalog, ordinals],
  )
}

export function useNativeSeethingSwarmActiveBattle(
  ...
  const ordinals = useSelector(actor, (snapshot) =>
    projectSeethingSwarmBattleOrdinals(snapshot?.context, identity, scope),
  )
  ...
  const onRoleEntered = useCallback(
    (animalId: ZooAnimalId, role: SeethingSwarmVariedRole) => {
      actor?.send({
