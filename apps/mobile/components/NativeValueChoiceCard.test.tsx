...
describe("NativeValueChoiceCard", () => {
  it("combines supported hover and focus responses without requiring an extra press", async () => {
    ...
    expect(choice).toHaveProp("accessibilityHint", getValueDisplayDefinition(selfAcceptance))
    const captionText = `↑ ${getValueDisplayName(selfAcceptance)}`
    expect(screen.queryByText(captionText)).toBeNull()
    const animalCaption = screen.getByText(captionText, { includeHiddenElements: true })
    await fireEvent(animalCaption, "pointerEnter", { nativeEvent: { pointerType: "mouse" } })
    expect(screen.getByText("Animal alert", { includeHiddenElements: true })).toBeOnTheScreen()
    await fireEvent.press(animalCaption)
    expect(onActivate).not.toHaveBeenCalled()
    await fireEvent(animalCaption, "pointerLeave")
    expect(screen.getByText("Animal resting", { includeHiddenElements: true })).toBeOnTheScreen()
