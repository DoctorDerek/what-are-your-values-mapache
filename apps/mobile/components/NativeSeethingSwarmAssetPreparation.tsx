...
export default function NativeSeethingSwarmAssetPreparation({
  children,
}: {
  children: ReactNode
}) {
    ...
    <PreparationContext value={actor}>
      ...
      >
        {[...assets.values()].map(({ clip, generation }) => (
          <Image
            key={generation}
            testID={`prepared-animal-${generation}`}
            source={clip.asset}
            alt=""
            accessible={false}
            fadeDuration={0}
...
export function usePreparedNativeSeethingSwarmClips(
  clips: readonly SeethingSwarmRuntimeCharacterClip<number>[],
) {
...
export function usePreparedNativeSeethingSwarmBattle(
