...
export function getHubPreparationClips<Asset>(
  rankedValues: readonly RankedValue[],
  catalog: SeethingSwarmRuntimeClipCatalog<Asset>,
) {
  const { topFive } = projectHubValues(rankedValues)
...
export function isChoreographyPrepared<Asset>(
  choreography: SeethingSwarmBattleChoreography<Asset>,
  ...
  return choreography.combatants.every((combatant) => {
    ...
    if (
      initialClips.some(
        (clip) => assets.get(clip.relativePath)?.status === "ready",
      )
    )
      return true
    return (
      initialClips.every(
        (clip) => assets.get(clip.relativePath)?.status === "failed",
      ) &&
    ...
    on: {
      ...
      "ASSETS.RELEASED": {
        actions: assign(({ context, event }) => {
          ...
          return {
            scopes,
            assets: new Map(
              [...context.assets].filter(([path]) => retainedPaths.has(path)),
            ),
          }
        }),
      },
      "ASSET.SETTLED": {
        actions: assign(({ context, event }) => {
          ...
          if (
            !asset ||
            asset.generation !== event.generation ||
            asset.status !== "pending"
          )
            return {}
          return {
            assets: new Map(context.assets).set(event.path, {
              ...asset,
              status: event.status,
            }),
          }
